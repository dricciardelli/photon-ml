...
class LocalDataSetTest {
  ...
  def dataForRandomEffectFeatureSelection(): Array[Array[Any]] ={
    ...
    // case 1 for x = 0, y = 0
    // case 2 for x = 0, y != 0
    // case 3 for nonBinary
    // case 4 for x = m, y = n
    // case 5 for t > 1 not selected
    // case 6 for t > 1 selected
    // case 7 for t < 1 not selected
    // case 8 for t < 1 selected
    ...
    val x_0_y_not_0_expected = Map(0 -> 0.122811936)

    val x_0_y_not_0_stats = Array(50.0)

    val x_0_y_not_0_g_positive = Array(20.0)
    ...
    val nonBinary_stats = Array(25.0, 0.0)
    ...
    val x_m_y_n_expected = Map(0 -> 0.0)

    val x_m_y_n_stats = Array(50.0)
    ...
    val t_bt_1_not_selected_stats = Array(50.0)
    ...
    val t_bt_1_selected_stats = Array(50.0)
    ...
    val t_lt_1_not_selected_stats = Array(50.0)
    ...
    val t_lt_1_selected_stats = Array(100.0)
  ...
  @Test(dataProvider = "dataForRandomEffectFeatureSelection")
  def testFilterFeaturesByRatioCIBound(
    ...
    val filteredDataPoints = localDataSet
      .filterFeaturesByRatioCIBound(globalFeatureInstances, globalPositiveInstances, binaryIndices, nonbinaryIndices)
      .dataPoints
  ...
  def testComputeRatioCILowerBound(
    ...
    computed.foreach { case (key, value) =>
      assertEquals(
        ...
        expected(key),
        CommonTestUtils.LOW_PRECISION_TOLERANCE,
        s"Computed Ratio Confidence Interval LowerBound score is $value, while the expected value is ${expected(key)}")
    }
  }
  ...
  /**
   * Test the lower bound and upper bound computation
   */
  @Test
  def testUpperBoundAndLowerBound(): Unit = {
    ...
    val lowerBound = LocalDataSet.computeLowerBound(t, variance, zScore)
    val upperBound = LocalDataSet.computeUpperBound(t, variance, zScore)
